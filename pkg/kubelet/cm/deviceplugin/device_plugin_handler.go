...
// Start init allocatedDevices information and starts device plugin registration service.
...
// allocateContainerResources attempts to allocate all of required extended
// resources for the input container, issues an Allocate rpc request for
// each of such resources, processes their AllocateResponses, and updates
// the cached containerDevices on success.
...
// Allocate attempts to allocate all of required extended resources,
...
// and update allocatableResource in nodeInfo if necessary
func (h *HandlerImpl) sanitizeNodeAllocatable(node *schedulercache.NodeInfo) {
	...
	for resource, devices := range h.allocatedDevices {
		...
		// NodeInfo.allocatableResource at least equal to the pod's requested capacity
